...
if [[ "${1:-}" == "--all" ]]; then
    ...
    for node in lj1.zeyu.tw lj2.zeyu.tw lj3.zeyu.tw; do
        echo "[${node}]"
        ssh -o ConnectTimeout=5 "zeyu@${node}" "docker exec fe_rnic bash -c 'source /home/zeyu/vLLM/v0.11.0/.venv/bin/activate && cd /home/zeyu/vLLM/v0.11.0/fe_rnic && bash clean.sh'" 2>/dev/null || echo "  连接失败: $node"
        echo ""
    done
    echo "========== 全部清理完成 =========="
else
